//modify this to request forecast date and save requested dates into archive by date
...
// console.log('date', makeDates(1,'JAN',17,2,'JUL',17,3,'JUL',17,))
async function makeDugReq(){
  ...
  if(curHr<18){
    ...
    var day = h0.getDate()
    var mon = monthAr[h0.getMonth()]
    var yr = h0.getFullYear() - 2000

    // console.log(day, mon, yr)
	// const dates = await makeDates(day1,month1,year1,day2,month2,year2,day3,month3,year3);
  // const reqAr = [[dates[0],'F'], [dates[1], 'R'], [[dates[2], 'R']], [dates[3], 'R']]    
      // console.log('dates', dates, reqAr)
	// const axiosArray = []
  var url = `https://fam.nwcg.gov/wims/xsql/nfdrs.xsql?stn=&sig=DUGWAY&type=O,R&start=1-JAN-16&end=${day}-${mon}-${yr}&time=&priority=&fmodel=7G&sort=asc&ndays=&user=679`
  axios.get(url)
  
	// dates.map((curr, i)=>{
	// 	const url = 'https://fam.nwcg.gov/wims/xsql/nfdrs.xsql?stn=&sig=ALL_GB&type=' + reqAr[i][1] + '&start='+ curr + '&end=' + curr + '&time=&user=679&fmodel=7G';
  ...
  // var addObj = { ...fdraInfo[currFdra], ...returnText}
	.then((values)=>{
      ...
      console.log('here')
		...
		// 	const inc = i+1;
      var dateAr = []
			parseString(values.data, function (err, result) {
        var resultAr = [];
        ...
        keyAr.map((curr)=>{
         ...
         // console.log('entry', entry)
           ...
           var id = entry.sta_id[0] ? entry.sta_id[0] : 'missing'
           var stnId = entry.sta_id[0] ? entry.sta_id[0] : 'missing'
           var erc = entry.ec ? entry.ec[0] : 'missing'
           var bi = entry.bi ? entry.bi[0] : 'missing'
           var date = entry.nfdr_dt ? entry.nfdr_dt[0] : 'missing'
           console.log(erc, bi, 'here')
           if(erc!== 'missing' && bi !== 'missing'){
             console.log('in here')
            var percentiles = calcPercentile({stnId, erc, bi})
            console.log('percentiles', percentiles)
           }
           dateAr.push(entry.nfdr_dt[0])
           
         }
         // console.log('percentiles', percentiles)
         if(percentiles){
            // console.log('percentiles', percentiles)
            if(!percentiles.jolInd){
              // console.log(entry.nfdr_dt, percentiles.jolInd, 'erc', percentiles.erc, 'bi',percentiles.bi)
            }
            if(!ercObj[entry.nfdr_dt[0]]){

             ercObj[entry.nfdr_dt[0]]={}
            }
             ercObj[entry.nfdr_dt[0]][id]={
             "name" : entry.sta_nm[0],
             ...
             "bi" : entry.bi[0],
             "adj" : entry.adj[0],
             "ercPercentile": percentiles.erc,
             "biPercentile" : percentiles.bi,
             "jolInd" : percentiles.jolInd,
             "fcstBiPerc" : percentiles.fcstBiPerc,
             "fcstErcPerc" : percentiles.fcstErcPerc,
             "fcstIndexPerc" : percentiles.fcstIndexPerc, 
             "swfpiFcst" : percentiles.swfpiFcst
             }
           }
        ...
        // console.log('dateeAr', dateAr)
        var dateSet = new Set(dateAr)
        // console.log('dateSet', dateSet)
        // console.log('ercObj', ercObj)
        var save = JSON.stringify(ercObj);
        fs.writeFile(`./data/archive/dugwayArchive.json`, save, function (err) {
         // console.log(ercObj)    
  ...
  //     	var erc = entry.ec[0];
  //     	var bi = entry.bi[0];
  //     	var percentiles = calcPercentile({id, erc, bi})
  //     	if(percentiles){
  //     	     console.log('percentiles', percentiles)
  //     		ercObj[id]={
  //     		"name" : entry.sta_nm[0],
...
// });
// var ercObj = {};
// axios.get('https://fam.nwcg.gov/wims/xsql/nfdrs.xsql?stn=&sig=ALL_GB&type=R&start=1-JUL-17&end=1-JUL-17&time=&user=679&fmodel=7G')
//   .then(response => {
//     var self = this; 
